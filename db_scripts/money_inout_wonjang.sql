create or alter procedure MONEY_INOUT_WONJANG (
    DONG_ID varchar(17),
    START_DATE date,
    END_DATE date)
returns (
...
begin
  ...
  from MONEY_INOUT  where IO_DONG_ID = :DONG_ID and IO_DATE < :start_date
  into :V_IN, :V_OUT ;
  IO_JUKYO = '''';
  IO_IN = V_IN;
  IO_OUT = V_OUT;
  ...
  for select ID, IO_DATE, IO_ITEM_ID, IO_SUBITEM_ID, IO_DETAIL_ID, IO_JUKYO, IO_IN, IO_OUT, IO_DONG_ID, IO_KIND, IO_REG_LECTURE_ID
      from MONEY_INOUT where (IO_DONG_ID = :DONG_ID) and (IO_DATE between :start_date and :end_date)
      order by IO_DATE
